...
function nextMonth(now) {
    ...
    if (!relative) {
        var _month = (now.getMonth() + 12 + index) % 12;
        var _dayOfMonth = Math.min(now.getDate(), daysInMonth(year, _month));
        return new Date(now.getFullYear(), _month, _dayOfMonth, now.getHours(), now.getMinutes(), now.getSeconds());
    }
    return new Date(year, month, dayOfMonth, now.getHours(), now.getMinutes(), now.getSeconds());
}
